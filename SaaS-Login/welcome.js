...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (status.toLowerCase() === 'activo' && destinationUrl) {
            const btn = document.createElement('button'); // Changed to button
            ...
            // OPEN PORTAL LOGIC (Nueva Pestaña con Token Masking)
            btn.onclick = () => {
                // Abrir en nueva pestaña usando la URL enmascarada con token
                // El token expira en 10 min y solo funciona 1 vez
    ...
    document.getElementById('closePortalBtn').addEventListener('click', () => {
        ...
        // Clear src to stop video/audio or heavy scripts
        portalIframe.src = '';
    });
